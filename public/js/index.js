...
let initSmoothScrollingMain = () => {
	...
	let scrollFn = (time) => {
		lenisMain.raf(time); // Run Lenis' requestAnimationFrame method
...
let applyAnimation = (grid, animationType) => {
	// Child elements of grid
	let gridWrap = grid.querySelector('.grid-wrap');
	let gridItems = grid.querySelectorAll('.grid__item');
	let gridItemsInner = [...gridItems].map(item => item.querySelector('.grid__item-inner'));
...
preloadImagesHome('.grid__item-inner').then(() => {
	initSmoothScrollingMain();
	scroll();
});